...
class OpenfedValidations {
  ...
  private static function isDrupalSite() {
    ...
    if (empty($output) || $output != 'Successful') {
      print_r("## This is not a valid Drupal site or it's not properly installed yet.\n");
      return FALSE;
    }
    return TRUE;
  }
  ...
  private static function checkDeprecatedThemes($arguments = []) {
    ...
    if (empty($cget_system_theme)) {
      print_r("## Not possible to check Admin Theme because this is not a valid Drupal site or it's not properly installed yet.\n");
    } else {
